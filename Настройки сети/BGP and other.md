...
Для проверки того, что соединения установлены, можем ввести команду `show ip bgp summary`. Пример вывода с R2 на скриншоте: 

<img width="960" height="716" alt="2025-11-09_20-58-30" src="https://github.com/user-attachments/assets/c6874196-2a3b-4d46-b9c0-cf3b2ec7d734" />

## Добавляем PC

**Настройка R1:**
```bash
R1(config)#interface e1/1 !Настраиваем интерфейс между R1 и VPC1
R1(config-if)#ip address 10.1.1.1 255.255.255.0 !Задаем локальную сеть
R1(config-if)#no shutdown !Подымаем сеть

R1(config)#router bgp 64512 !Переключаемся на настройку BGP 
R1(config-router)#network 10.1.1.0 mask 255.255.255.0 !Анонсируем сеть VPC1 
```
...
***Настройка VPC 1:***
```bash
VPC1> ip 10.1.1.2/24 10.1.1.1 !Задали ip-адрес, сеть и шлюз по умолчанию
Checking for duplicate address... !Система проверяет на дотупность данного адреса в сети
VPC1 : 10.1.1.2 255.255.255.0 gateway 10.1.1.1 !Система назначила данный адрес за этой машиной
```
***Настройка VPC 3:***
```bash
VPC3> ip 10.3.3.2/24 10.3.3.1
Checking for duplicate address...
VPC3 : 10.3.3.2 255.255.255.0 gateway 10.3.3.1
```

### Проверка

Для проверки пытемся пингануть VPC3 с VPC1. И как видим - успешно. 

<img width="1013" height="719" alt="2025-11-09_21-52-37" src="https://github.com/user-attachments/assets/370193d3-cccf-4e35-babd-4565c49681cf" />
...
3. Добавил блок с простой настройкой

Планируемые блоки: 
1. Настройкка internal BGP
2. Добавить скриншоты с WS.
